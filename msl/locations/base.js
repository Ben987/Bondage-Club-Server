...
Location.prototype.ActionStart_AppearanceUpdate = function(player, data){
	this.ValidatePlayerNotActing(player, data.type);
	var targetPlayer = data.targetPlayerId == player.id ? player : Msl.GetPlayer(data.targetPlayerId);
	Assets.UpdateAppearance(data.appearanceUpdate, targetPlayer, player);
	return new InstantAction(player, "AppearanceUpdate", null, targetPlayer, data.appearanceUpdate);
}
