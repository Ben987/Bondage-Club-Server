...
function ChatRoomSearch(data, socket) {
	...
	for (const room of rooms) {
		...
		// If query isn't the exact name and player isn't in visibililty list, skip
...
function ChatRoomCreate(data, socket) {
	...
	if ((data != null) && (typeof data === "object") && (data.Name != null) && (data.Description != null) && (data.Background != null) && (typeof data.Name === "string") && (typeof data.Description === "string") && (typeof data.Background === "string")) {
		{ // BACKWARD-COMPATIBILITY BLOCK for Private/Locked Transition
			...
			} else if (data.Private != null) { // Help old clients add Visibility for new clients | private = strictest visibility
				data.Visibility = data.Private ? [] : ["All"];
			}
			...
			} else if (hasLocked) { // Help old clients add Access if Locked is set | locked = strictest access
				data.Access = data.Locked ? ["Admin"] : ["All"];
					...
					if (data.Room.Name.match(ServerChatRoomNameRegex) && (data.Room.Description.length <= 100) && (data.Room.Background.length <= 100)) {
						...
						{ // BACKWARD-COMPATIBILITY BLOCK for Private/Locked Transition
							if (data.Room.Visibility != null && Array.isArray(data.Room.Visibility)) {
								...
								data.Room.Visibility = data.Room.Private ? [] : ["All"]; // Help old clients add Visibility for new clients | private = strictest visibility
							}
							...
							} else if (data.Room.Locked != null && typeof data.Room.Locked === "boolean") { // Help old clients add Access for new clients | locked = strictest access
								data.Room.Access = data.Room.Locked ? ["Admin"] : ["All"];
							}
						}
						...
						if ((Acc != null) && (Acc.ChatRoom != null)) {
							...
							Dictionary.push({Tag: "ChatRoomPrivacy", TextToLookUp: (ChatRoomRoleListIsRestrictive(Acc.ChatRoom.Visibility) ? "Public" : "Private")});
							Dictionary.push({Tag: "ChatRoomLocked", TextToLookUp: (ChatRoomRoleListIsRestrictive(Acc.ChatRoom.Access) ? "Unlocked" : "Locked")});
